/* ==========================================================================
   Interactive Engine for CryptoCurrent
   ========================================================================== */

// Base Simulated Article Database
const localArticles = [
    {
        "title": "Layer-2 Scaling Solutions: Ethereum's Data Gas Breakthrough",
        "excerpt": "With the implementation of Blob transactions, layer-2 rollup networks have reduced transaction fees by up to 90%. What this means for DeFi.",
        "category": "Ethereum",
        "author": "Satoshi Vance",
        "date": "1 hour ago",
        "readTime": "5 min read",
        "body": "<p>Ethereum's layer-2 scaling ecosystems have achieved significant efficiency gains following recent database upgrades. The implementation of Blob transactions allows networks to store data in dedicated off-chain memory units, bypassing expensive standard storage routes.</p><p>This shift has driven average transaction costs on rollups down to fractions of a cent, enabling high-frequency trading and Web3 micropayments that were previously impractical.</p><p>DeFi developers are deploying complex smart contract models that take advantage of these lower fees, accelerating user adoption.</p>",
        "premium": true
    },
    {
        "title": "Real-World Asset Tokenization: Corporate Finance Moves On-Chain",
        "excerpt": "Global investment banks are deploying pilot projects to tokenize commercial paper, mortgages, and treasury bills on public ledgers.",
        "category": "Web3 Strategy",
        "author": "Elena Rostova",
        "date": "4 hours ago",
        "readTime": "6 min read",
        "body": "<p>Asset tokenization\u2014representing ownership of physical assets as digital tokens on a blockchain\u2014is gaining corporate adoption.</p><p>Major financial institutions are launching initiatives to put treasury bonds, commercial real estate, and trade invoice files on public or permissioned ledgers. Tokenization simplifies settlement times, eliminates intermediaries, and increases asset accessibility.</p><p>Regulators are working to establish frameworks to govern these on-chain assets, balancing technological innovation against compliance requirements.</p>",
        "premium": false
    },
    {
        "title": "Decentralized Finance: Yield Strategies in the Current Cycle",
        "excerpt": "We analyze liquidity pooling, automated market makers, and low-risk stablecoin lend rates for the upcoming quarter.",
        "category": "DeFi Yields",
        "author": "David Chen",
        "date": "1 day ago",
        "readTime": "5 min read",
        "body": "<p>DeFi ecosystems are offering attractive yield opportunities for capital allocators. As trading activity increases, automated market makers (AMMs) are delivering consistent fee generation to liquidity providers.</p><p>We review key yield-generation strategies:</p><ul><li><b>Liquidity Pools:</b> Providing dual-token collateral to earn trading fees, balancing risks of impermanent loss.</li><li><b>Lending Protocols:</b> Depositing stablecoins to earn interest from borrowers, presenting low risk parameters.</li><li><b>Liquid Staking:</b> Earning native network validation yields while maintaining capital liquidity through derivative tokens.</li></ul>",
        "premium": true
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isArticlePage) {
        if (artId !== null) {
            ...
            const savedRssArt = localStorage.getItem('7_crypto_current_active_rss_article');
            const savedRssList = localStorage.getItem('7_crypto_current_rss_articles');
            if (savedRssList) {
...
function loadStateFromStorage() {
    const savedTheme = localStorage.getItem('7_crypto_current_dark_mode');
    state.isDarkMode = savedTheme === 'true';

    const savedBookmarks = localStorage.getItem('7_crypto_current_bookmarks');
    if (savedBookmarks) {
        state.bookmarks = JSON.parse(savedBookmarks);
    }

    const savedUpvotes = localStorage.getItem('7_crypto_current_upvotes');
    if (savedUpvotes) {
        state.upvoted = JSON.parse(savedUpvotes);
    }

    const savedComments = localStorage.getItem('7_crypto_current_comments');
    if (savedComments) {
        state.comments = JSON.parse(savedComments);
    }

    const savedSub = localStorage.getItem('7_crypto_current_subscribed');
    state.isSubscribed = savedSub === 'true';

    const savedViews = localStorage.getItem('7_crypto_current_pageviews');
    if (savedViews) state.pageViews = parseInt(savedViews);

    const savedClicks = localStorage.getItem('7_crypto_current_adclicks');
    if (savedClicks) state.adClicks = parseInt(savedClicks);

    const savedEarnings = localStorage.getItem('7_crypto_current_earnings');
...
function saveStateToStorage() {
    localStorage.setItem('7_crypto_current_dark_mode', state.isDarkMode);
    localStorage.setItem('7_crypto_current_bookmarks', JSON.stringify(state.bookmarks));
    localStorage.setItem('7_crypto_current_upvotes', JSON.stringify(state.upvoted));
    localStorage.setItem('7_crypto_current_comments', JSON.stringify(state.comments));
    localStorage.setItem('7_crypto_current_subscribed', state.isSubscribed);
    localStorage.setItem('7_crypto_current_pageviews', state.pageViews);
    localStorage.setItem('7_crypto_current_adclicks', state.adClicks);
    localStorage.setItem('7_crypto_current_earnings', state.earnings.toFixed(2));
}
...
function renderTicker() {
    ...
    const items = ["BTC: $98,420 (+2.4%) | ETH: $3,640 (-0.8%) | SOL: $184.20 (+5.6%) | GAS: 12 Gwei", "Decentralized exchange volume sets a record high of $42 Billion in weekly trades", "Ethereum layer-2 network gas costs drop 90% following blob space implementation", "Web3 developer indices show active GitHub commits up 18% in layer-2 ecosystems", "Global regulators coordinate compliance frameworks for asset tokenization"];
...
function handleArticleClick(article) {
    ...
    if (localIdx > -1) {
        ...
        localStorage.setItem('7_crypto_current_active_rss_article', JSON.stringify(article));
...
function runWorkspaceCalculator() {
    const resultEl = document.getElementById('calc-results');
            ...
            const gasPrice = parseFloat(document.getElementById('gas-price-gwei').value) || 10;
            const txType = document.getElementById('tx-type').value;
            
            let gasLimit = 21000;
            let coinPrice = 3600; // ETH default
            let multiplier = 1.0;
            let netLabel = 'ETH';
            
            if (network === 'eth') {
                gasLimit = txType === 'swap' ? 120000 : 21000;
                coinPrice = 3600;
                netLabel = 'ETH';
            } else if (network === 'sol') {
                gasLimit = 5000;
            ...
            // Calculate gas fee in coin and convert to USD
            let feeInCoin = 0;
            if (network === 'sol') {
                feeInCoin = 0.000005 * (txType === 'swap' ? 3 : 1);
            } else {
                feeInCoin = (gasLimit * gasPrice) / 1000000000;
            }
            
            const feeUSD = feeInCoin * coinPrice;
            
            resultEl.innerHTML = `
                <div class="calc-result-box" style="border-left: 4px solid var(--primary); padding-left: 10px; margin-top: 15px;">
                    <h5 style="margin: 0 0 5px 0;">Estimated Gas Fee:</h5>
                    <div style="font-size: 24px; font-weight: bold; color: var(--primary);">$${feeUSD.toFixed(4)} <span style="font-size: 14px; font-weight: normal; color: var(--text-color);">USD</span></div>
                    <div style="font-size: 11px; margin-top: 5px; opacity: 0.8;">Network Cost: <strong>${feeInCoin.toFixed(6)} ${netLabel}</strong></div>
                    <p style="font-size: 10px; margin: 8px 0 0 0; opacity: 0.7; font-style: italic;">*Fees fluctuate dynamically with network traffic.</p>
                </div>
            `;
...
<a href="${link}" target="_blank" class="hero-btn" style="display:inline-block; margin-top:10px;">Read Original Source <i class="fa-solid fa-external-link"></i></a>`
            ...
            localStorage.setItem('7_crypto_current_rss_articles', JSON.stringify(parsedArticles));
